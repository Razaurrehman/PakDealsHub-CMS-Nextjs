...
type CatForm = z.infer<typeof categorySchema>;
...
export default function CategoriesPage() {
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = useForm<CatForm>({ resolver: zodResolver(categorySchema) as any });
  ...
  const openCreate = () => { reset({ name: '', isFeatured: false, isActive: true, sortOrder: 0 }); setEditing(null); setOpen(true); };
  const openEdit = (c: any) => { reset({ ...c, parentId: c.parentId ?? '', emoji: c.emoji ?? '' }); setEditing(c); setOpen(true); };
  ...
  const bulkDelete = async () => {
    if (!confirm(`Delete ${selectedIds.length} categor${selectedIds.length !== 1 ? 'ies' : 'y'}?`)) return;
    setDeleting(true);
    const res = await fetch('/api/categories', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: selectedIds }) });
    const json = await res.json();
    setDeleting(false);
    if (!json.ok) { toast(json.error ?? 'Error', 'error'); return; }
    toast(`Deleted ${json.data.deleted} categories`, 'success');
    load(page, debouncedSearch);
  };

  const columns = [
    { key: 'image', header: '', className: 'w-12', render: (c: any) => c.image ? <img src={c.image} className="h-8 w-8 rounded object-cover" /> : <span className="text-xl">{c.emoji ?? '📦'}</span> },
    ...
    { key: 'parent', header: 'Parent', render: (c: any) => c.parent?.name ?? '—' },
    { key: 'emoji', header: 'Emoji', render: (c: any) => c.emoji ?? '—' },
    { key: 'isFeatured', header: 'Featured', render: (c: any) => c.isFeatured ? <Badge variant="info"><Star size={10} className="inline mr-1" />Featured</Badge> : <span className="text-muted-foreground text-xs">—</span> },
  ...
  return (
    <>
      <PageHeader title="Categories" description="Organize products by category" action={{ label: 'Add Category', onClick: openCreate }} />
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Edit Category' : 'Add Category'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          <div className="border-t border-border pt-5 space-y-3">
            ...
            <ImageUpload label="Image" value={watch('image') ?? ''} onChange={v => setValue('image', v)} />
            <div className="grid grid-cols-3 gap-4">
              <Input label="Icon" placeholder="icon-name" {...register('icon')} />
              <Input label="Emoji" placeholder="📱" {...register('emoji')} />
              <Input label="Box Color" placeholder="#3B82F6" {...register('boxColor')} />
            </div>
          </div>
          ...
          <div className="border-t border-border pt-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Select label="Parent Category" options={categories.filter(c => c.id !== editing?.id).map(c => ({ value: c.id, label: c.name }))}
                placeholder="— None —" value={watch('parentId') ?? ''} onChange={e => setValue('parentId', e.target.value || null)} />
